...
$$\hE{Y^a} = \sum_{\ell} \hE{Y \mid A = a, L = \ell} \times \hat{\Pr}[L = \ell]$$
...
$$\hE{Y^{a=1}} = \sum_{\ell=1}^k \hat{\Pr}[Y = 1 \mid A = 1, L = \ell] \times \hat{\Pr}[L = \ell]$$

$$\hE{Y^{a=0}} = \sum_{\ell=1}^k \hat{\Pr}[Y = 1 \mid A = 0, L = \ell] \times \hat{\Pr}[L = \ell]$$

**Causal effect**: $\hE{Y^{a=1}} - \hE{Y^{a=0}}$
...
Given a model $\hE{Y \mid A, L}$, the **parametric g-formula** estimates:

$$\hE{Y^a} = \frac{1}{n} \sum_{i=1}^n \hE{Y \mid A = a, L = L_i}$$

**Algorithm**:

1. Fit outcome model $\hE{Y \mid A, L}$ using all data
2. For each individual $i$, predict $\hat{Y}_i^a = \hE{Y \mid A = a, L = L_i}$
3. Average the predictions: $\hE{Y^a} = n^{-1} \sum_i \hat{Y}_i^a$
...
4. Average: $\hE{Y^{a=1}} = \bar{\hat{Y}}^{a=1}$, $\hE{Y^{a=0}} = \bar{\hat{Y}}^{a=0}$
5. Estimate causal effect: $\hE{Y^{a=1}} - \hE{Y^{a=0}}$

::: {.notes}
**Why this works**: Under conditional exchangeability $Y^a \ind A \mid L$:
...
::: {#def-standardization-distributions}
## Reference Distributions for Standardization

The g-formula can standardize the mean outcome under treatment $a$ to different reference distributions of $L$:
...
The **average treatment effect in the treated (ATT)** is

$$\E{Y^{a=1} \mid A = 1} - \E{Y^{a=0} \mid A = 1},$$

standardized to the distribution of $L$ among the treated.
The g-formula estimator for the ATT is

$$\hE{Y^a \mid A = 1} = \frac{1}{n_1} \sum_{i: A_i = 1} \hE{Y \mid A = a, L = L_i},$$
...
1. Fit outcome model $\hE{Y \mid A, L}$ (e.g., linear regression)
2. For chosen dose $a$, predict $\hat{Y}_i^a = \hE{Y \mid A = a, L = L_i}$ for all $i$
3. Average: $\hE{Y^a} = n^{-1} \sum_i \hat{Y}_i^a$
...
- Estimate $\hE{Y^a}$ for $a = 0, 5, 10, 15, 20, \ldots$ cigarettes/day
- Plot $a$ vs $\hE{Y^a}$ to visualize dose-response
...
::: {#def-gformula-effect-measures}
## G-Formula Estimators of Causal Effect Measures
