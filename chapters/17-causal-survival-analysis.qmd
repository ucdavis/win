...
::: {.notes}
In our example, if we assume (provisionally) that quitters ($A = 1$) and non-quitters ($A = 0$) are marginally exchangeable, the survival curves at 120 months were 76.2% among quitters and 82.0% among non-quitters. Equivalently, the 120-month risk was 23.8% among quitters and 18.0% among non-quitters.
...
The discrete-time hazard ratio $\frac{\Pr[D_{k+1}=1|D_k=0,A=1]}{\Pr[D_{k+1}=1|D_k=0,A=0]}$ equals $\exp(\alpha_1)$ at all times $k+1$ in the hazards model $\Pr[D_{k+1} = 1 \mid D_k = 0, A] = \Pr[D_{k+1} = 1 \mid D_k = 0, A = 0] \times \exp(\alpha_1 A)$.
...
The validity of this procedure requires no misspecification of both the treatment model and the marginal hazards model.
:::
...
More generally, the effect of treatment may depend on covariates $L$ so a more general structural AFT would be $T^a_i / T^{a=0}_i = \exp(-\psi_1 a - \psi_2 a L_i)$. Rearranging, the model can be written as:

$$T^{a=0}_i = T^a_i \exp(\psi_1 a + \psi_2 a L_i) \quad \text{for all individuals } i$$

### Selection Bias from Administrative Censoring

Consider a 60-month randomized experiment. Table 17.1 shows the survival time under treatment and under no treatment for three types of individuals:
