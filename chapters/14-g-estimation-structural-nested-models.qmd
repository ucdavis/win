...
Under $H_0(\psi)$, the **pseudo-outcome** is
...
3. Under conditional exchangeability, $Y^0 \ind A \mid L$
4. So the correct $\psi$ makes $H(\psi)$ independent of $A$ conditional on $L$

This is the **estimating principle**: Find $\psi$ such that $H(\psi) \ind A \mid L$.
:::
...
**Step 3**: Test whether $H(\psi) \ind A \mid L$ by fitting
...
$$\sum_{i=1}^n [A_i - \hE{A \mid L_i}][Y_i - A_i(\psi_0 + \psi_1^{\top} L_i)] = 0$$
...
::: {#def-g-estimation-functions}
## Choice of Estimating Functions for G-Estimation
...
::: {#def-g-estimation-censoring}
## G-Estimation with Censoring Weights

Under the assumption $C \ind Y^a \mid A, L$ (censoring independent of potential outcomes given treatment and covariates), the **weighted estimating equation** is
