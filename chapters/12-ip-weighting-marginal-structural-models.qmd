...
::: {#def-ip-weights-censoring}
## IP Weight for Censoring

The **IP weight for censoring** is
...
When we have both confounding and censoring, the treatment and censoring weights are combined by multiplication:

::: {#def-joint-weights-treatment-censoring}
## Joint Weights for Treatment and Censoring
...
::: {#def-missing-at-random}
## Missing at Random (MAR)

Censoring is **missing at random (MAR)** if it is independent of the potential outcomes given the observed treatment and covariates, $C \ind Y^a \mid A, L$: censoring may depend on observed variables but not on the (unobserved) outcome.

MAR is weaker than **missing completely at random (MCAR)**, under which censoring does not depend on either observed or unobserved variables.
:::
...
::: {#def-weighted-likelihood}
## Weighted Estimating Equations and Weighted Likelihood
