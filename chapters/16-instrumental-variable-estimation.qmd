...
2. **Exchangeability**: $Z$ is independent of unmeasured confounders $U$ (i.e., $Y^{a,z} \ind Z$)
...
**Statement**: $Y^{a,z} \ind Z$ (or $Y^a \ind Z$ under exclusion)
...
- $Z \ind U$ where $U$ are unmeasured confounders
...
For a binary instrument $Z$ and binary treatment $A$, the **Wald estimator** (also called the **ratio estimator**) is
...
The **monotonicity assumption** states that no defiers exist in the population.
:::
...
Under the IV conditions plus monotonicity, the IV estimand equals

$$\E{Y^{a=1} - Y^{a=0} \mid \text{Complier}},$$

the **local average treatment effect (LATE)** — the average causal effect in compliers, not in the full population.
:::
...
- Need $Y^a \ind Z \mid L$ (conditional exchangeability of instrument)
...
Given measured covariates $L$, $Z$ is a **conditional instrumental variable** for the effect of $A$ on $Y$ if:

1. **Conditional relevance**: $Z \not\ind A \mid L$
2. **Conditional exchangeability**: $Y^a \ind Z \mid L$
...
- Assumes no unmeasured confounding: $Y^a \ind A \mid L$
