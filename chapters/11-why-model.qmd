...
- Predict for $A = 90$: $\hE{Y|A = 90} = 70 + 1.25(90) = 182.5$
...
::: {#def-kernel-smoothing}
## Kernel Smoothing

**Kernel smoothing** estimates $\E{Y|A = a}$ using a weighted average of nearby observations:

$$\hE{Y|A = a} = \frac{\sum_i K\left(\frac{A_i - a}{h}\right) Y_i}{\sum_i K\left(\frac{A_i - a}{h}\right)}$$

where $K(\cdot)$ is a **kernel function** (e.g., Gaussian) and $h$ is the **bandwidth**, which controls the amount of smoothing.
:::
