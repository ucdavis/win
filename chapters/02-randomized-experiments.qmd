...
::: {#thm-association-equals-causation}
## Association Equals Causation Under Exchangeability
...
::: {#def-randomization-types}
## Types of Conditional Randomization
...
In randomized experiments, individuals may not receive or adhere to their assigned treatment. For example, someone randomized to receive a transplant may refuse it.

Let $Z$ denote random assignment and $A$ denote treatment actually received.

::: {#def-itt-ch2}
## Intention-to-Treat (ITT) Effect

The **intention-to-treat (ITT) effect** is the causal effect of treatment *assignment* (not treatment received): $\E{Y^{z=1}} - \E{Y^{z=0}}$.
:::

::: {#def-pp-ch2}
## Per-Protocol Effect

The **per-protocol effect** is the causal effect of treatment actually received: $\E{Y^{a=1}} - \E{Y^{a=0}}$.
:::
...
## Permutation Test (Fisher's Randomization Test)

Under the sharp causal null hypothesis, **exact inference** can be performed using a **permutation test** (also called **Fisher's randomization test**), which is valid in finite samples without distributional assumptions.
:::
...
A causal quantity is **identified** if it can be expressed as a function of the observed data distribution.
:::

Exchangeability is a sufficient condition for identifying average causal effects.
...
\begin{align}
\E{Y^a} &= \E{\E{Y^a \mid L}} \quad \text{(law of total expectation)} \\
&= \E{\E{Y^a \mid A=a, L}} \quad \text{(by conditional exchangeability)} \\
&= \E{\E{Y \mid A=a, L}} \quad \text{(by consistency)} \\
&= \E{\frac{I(A=a)}{\Pr[A=a|L]} \E{Y \mid A=a, L}} \quad \text{(inverse weighting)} \\
