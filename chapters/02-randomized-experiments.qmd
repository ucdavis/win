...
Under conditional exchangeability $Y^a \perp\!\!\!\perp A | L$, the mean counterfactual outcome is:
...
1. **Conditional exchangeability**: $Y^a \perp\!\!\!\perp A | L$
...
1. **Randomization** ensures exchangeability $Y^a \perp\!\!\!\perp A$, allowing causal effects to be identified from associational contrasts

2. **Conditional randomization** achieves conditional exchangeability $Y^a \perp\!\!\!\perp A | L$ within strata, requiring adjustment methods
...
&= E\left[\frac{I(A=a)}{Pr[A=a|L]} E[Y|A=a, L]\right] \quad \text{(inverse weighting)} \\
&= E\left[\frac{I(A=a) \cdot Y}{Pr[A=a|L]}\right] \quad \text{(by law of total expectation)}
\end{align}
