...
1. **No confounding**: $Y^a \ind A$ (conditional exchangeability not needed)
...
::: {#thm-propensity-balancing}
## Propensity Score Balancing Property

If $Y^a \ind A \mid L$, then
...
::: {#alg-propensity-stratification}
## Propensity Score Stratification

1. Estimate propensity score $\hat{e}(L_i)$ for all individuals
2. Create strata (e.g., quintiles) of the propensity score
3. Within each stratum, compute $\hE{Y \mid A = a, \text{stratum } s}$
4. Standardize across strata:

$$\hE{Y^a} = \sum_{s=1}^S \hE{Y \mid A = a, \text{stratum } s} \times \Pr[\text{stratum } s]$$
:::
...
A **treatment model** is any model for $\Pr[A \mid L]$ (or $f(A \mid L)$ for non-binary $A$).
:::
...
$$\hE{Y^a} = \frac{1}{n}\sum_{i=1}^n \left[\frac{I(A_i = a) Y_i}{f(a \mid L_i)} - \frac{I(A_i = a) - f(a \mid L_i)}{f(a \mid L_i)} m(a, L_i)\right],$$

where $m(a, L) = \hE{Y \mid A = a, L}$ is the outcome model.
:::
...
::: {#def-generalized-propensity-score}
## Generalized Propensity Score
...
::: {#thm-gps-balancing}
## Generalized Propensity Score Balancing Property

Under conditional exchangeability,

$$Y^a \ind A \mid f(A \mid L)$$
:::
