...
Consider a simple causal diagram where $L$ causes both $A$ and $Y$, and $A$ causes $Y$:

```{r}
#| label: fig-basic-dag
#| fig-cap: "A simple causal DAG: $L$ causes $A$ and $Y$; $A$ causes $Y$."
#| fig-height: 3
dagify(
  Y ~ A + L,
  A ~ L,
  ...
  ggdag(node_size = 14, text_size = 4) +
  theme_dag()
```
...
#| fig-cap: "Causal DAG: exercise ($L$) causes smoking ($A$) and heart disease ($Y$); smoking causes heart disease."
#| fig-height: 3
dagify(
  Y ~ A + L,
  A ~ L,
  labels = c(A = "Smoking", L = "Exercise", Y = "Heart\ndisease"),
  coords = list(
    x = c(L = 0, A = 1, Y = 2),
    y = c(L = 1, A = 0, Y = 0)
  )
) |>
  ggdag(use_labels = "label", node_size = 14, text_size = 3) +
  theme_dag()
```
...
```{r}
#| label: fig-marginal-independence
#| fig-cap: "Four DAG structures illustrating marginal (in)dependence."
#| fig-height: 6
#| fig-width: 8
p1 <- dagify(Y ~ A,
  coords = list(x = c(A = 0, Y = 1), y = c(A = 0, Y = 0))
) |> ggdag(node_size = 12, text_size = 3.5) +
  theme_dag() + ggtitle("Diagram 1: A → Y")

p2 <- dagify(Y ~ 1, A ~ 1,
  coords = list(x = c(A = 0, Y = 1), y = c(A = 0, Y = 0))
) |> ggdag(node_size = 12, text_size = 3.5) +
  theme_dag() + ggtitle("Diagram 2: A   Y (no arrow)")

p3 <- dagify(Y ~ A, A ~ L,
  coords = list(x = c(L = 0, A = 1, Y = 2), y = c(L = 0, A = 0, Y = 0))
) |> ggdag(node_size = 12, text_size = 3.5) +
  theme_dag() + ggtitle("Diagram 3: Chain L → A → Y")

p4 <- dagify(Y ~ A + L,
  coords = list(x = c(A = 0, L = 1, Y = 2), y = c(A = 0, L = 1, Y = 0))
) |> ggdag(node_size = 12, text_size = 3.5) +
  theme_dag() + ggtitle("Diagram 4: Fork A ← L → Y")

(p1 + p2) / (p3 + p4)
```

- **Diagram 1** ($A \rightarrow Y$): $A$ and $Y$ are **dependent** (associated)
- **Diagram 2** ($A \quad Y$, no arrow): $A$ and $Y$ are **independent**
- **Diagram 3** ($L \rightarrow A \rightarrow Y$, chain): $A$ and $Y$ are **dependent**; $L$ and $Y$ are **dependent** (through the path $L \rightarrow A \rightarrow Y$)
- **Diagram 4** ($A \rightarrow Y \leftarrow L$, fork): $A$ and $L$ are **independent** (no common cause, no direct connection); both $A$ and $L$ are **dependent** with $Y$
:::
...
#| fig-cap: "The three fundamental DAG structures: chain, fork, and collider."
#| fig-height: 3
#| fig-width: 9
p_chain <- dagify(Y ~ Z, Z ~ X,
  coords = list(x = c(X = 0, Z = 1, Y = 2), y = c(X = 0, Z = 0, Y = 0))
) |> ggdag(node_size = 12, text_size = 3.5) +
  theme_dag() + ggtitle("Chain: X → Z → Y")

p_fork <- dagify(X ~ Z, Y ~ Z,
  coords = list(x = c(X = 0, Z = 1, Y = 2), y = c(X = 0, Z = 1, Y = 0))
) |> ggdag(node_size = 12, text_size = 3.5) +
  theme_dag() + ggtitle("Fork: X ← Z → Y")

p_collider <- dagify(Z ~ X + Y,
  coords = list(x = c(X = 0, Z = 1, Y = 2), y = c(X = 0, Z = 0, Y = 0))
) |> ggdag(node_size = 12, text_size = 3.5) +
  theme_dag() + ggtitle("Collider: X → Z ← Y")
...
#| fig-cap: "Collider DAG: natural ability ($A$) and training effort ($E$) are independent causes of becoming a professional athlete ($Y$)."
#| fig-height: 3
dagify(
  Y ~ A + E,
  labels = c(A = "Ability", E = "Effort", Y = "Pro athlete"),
  coords = list(
    x = c(A = 0, E = 2, Y = 1),
    y = c(A = 0, E = 0, Y = -1)
  )
) |>
  ggdag_collider(use_labels = "label", node_size = 14, text_size = 3) +
  theme_dag()
```
...
#| fig-cap: "Confounding DAG: $L$ is a common cause of $A$ and $Y$, creating a backdoor path $A \\leftarrow L \\rightarrow Y$."
#| fig-height: 3
dagify(
  Y ~ A + L,
  A ~ L,
  ...
  ggdag_paths(node_size = 14, text_size = 4) +
  theme_dag()
```
...
```{r}
#| label: fig-backdoor-criterion
#| fig-cap: "Backdoor criterion: $L$ blocks the only backdoor path from $A$ to $Y$. Adjusting for $L$ identifies the causal effect."
#| fig-height: 3.5
dagify(
  Y ~ A + L,
  A ~ L,
  ...
  ggdag_adjustment_set(node_size = 14, text_size = 4) +
  theme_dag()
```
...
#| fig-cap: "Selection bias: conditioning on $S$ (a collider caused by $A$ and $Y$) induces spurious association between $A$ and $Y$."
#| fig-height: 3
dagify(
  S ~ A + Y,
  labels = c(A = "A", S = "Selection (S)", Y = "Y"),
  coords = list(
    x = c(A = 0, S = 1, Y = 2),
    y = c(A = 0, S = -1, Y = 0)
  )
...
```{r}
#| label: fig-measurement-bias
#| fig-cap: "Measurement bias: using $A_{\\text{measured}}$ instead of $A_{\\text{true}}$ introduces bias."
#| fig-height: 3
dagify(
  Y ~ A_true,
  A_meas ~ A_true,
  labels = c(A_true = "A (true)", A_meas = "A (measured)", Y = "Y"),
  coords = list(
    x = c(A_true = 0, A_meas = 0, Y = 2),
    y = c(A_true = 0, A_meas = -1, Y = 0)
  )
) |>
  ggdag(use_labels = "label", node_size = 14, text_size = 3) +
  theme_dag()
...
```{r}
#| label: fig-effect-modification
#| fig-cap: "Effect modification: $V$ and $A$ both affect $Y$. The arrow $A \\to Y$ may differ in magnitude across levels of $V$."
#| fig-height: 3
dagify(
  Y ~ A + V,
  coords = list(
...
```{r}
#| label: fig-confounder-modifier
#| fig-cap: "Three scenarios for variable $V$ relative to $A \\to Y$: confounder only (left), modifier only in RCT (center), and both (right)."
#| fig-height: 3
#| fig-width: 9
p_conf <- dagify(
  Y ~ A + V,
  ...
  exposure = "A",
  outcome = "Y",
  coords = list(x = c(V = 0, A = 1, Y = 2), y = c(V = 1, A = 0, Y = 0))
) |>
  ggdag_status(node_size = 12, text_size = 3.5) +
  theme_dag() +
  ggtitle("Scenario 1: V confounder") +
...
p_mod <- dagify(
  ...
  exposure = "A",
  outcome = "Y",
  coords = list(x = c(V = 0, A = 0, Y = 2), y = c(V = 1, A = 0, Y = 0.5))
) |>
  ggdag_status(node_size = 12, text_size = 3.5) +
  theme_dag() +
  ggtitle("Scenario 2: V modifier (RCT)") +
  theme(legend.position = "none")
...
p_both <- dagify(
  ...
  ggdag_status(node_size = 12, text_size = 3.5) +
  theme_dag() +
  ggtitle("Scenario 3: V both") +
  theme(legend.position = "none")

p_conf + p_mod + p_both
```

**Scenario 1**: $V$ is a confounder only ($V \rightarrow A$, $V \rightarrow Y$). $V$ opens the backdoor path $A \leftarrow V \rightarrow Y$ (confounding). We must adjust for $V$.

**Scenario 2**: $V$ is a modifier only (in a randomized trial, no $V \rightarrow A$ arrow). $V$ modifies the effect of $A$, but does not confound (no backdoor path). We should report stratum-specific effects but don't need to adjust for $V$ to eliminate bias.

**Scenario 3**: $V$ is both a confounder and modifier. We must adjust for $V$ (confounding) AND report stratum-specific effects (modification).
:::
