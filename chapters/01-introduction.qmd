...
To define a causal effect, we need to compare two counterfactual outcomes for each individual.

::: {#def-potential-outcome}
## Counterfactual (Potential) Outcomes

For each individual, the **counterfactual outcomes** (also called **potential outcomes**) are:
...
For example, if Zeus would die if transplanted ($Y^{a=1} = 1$) but survive if not transplanted ($Y^{a=0} = 0$), then heart transplant has a causal effect on Zeus's outcome.
...
::: {#def-fundamental-problem}
## The Fundamental Problem of Causal Inference

The **fundamental problem of causal inference** is that we can only observe one of the two counterfactual outcomes for each individual.
:::

If Zeus receives a heart transplant, we observe $Y^{a=1}$ but not $Y^{a=0}$. The unobserved counterfactual outcome remains unknown.

::: {#def-consistency-ch1}
## Consistency

Formally, for each individual, the observed outcome $Y$ equals the counterfactual outcome $Y^a$ corresponding to the treatment actually received:

$$Y = Y^A.$$

This equality is called **consistency**.
:::
...
In our population, treatment does *not* have an average causal effect because both risks equal 0.5. However, this does not mean there are no individual effects.

::: {#def-average-causal-null}
## Null Hypothesis of No Average Causal Effect

The **null hypothesis of no average causal effect** (also called the **average causal null hypothesis**) holds when $\Pr[Y^{a=1} = 1] = \Pr[Y^{a=0} = 1]$, or equivalently $\E{Y^{a=1}} = \E{Y^{a=0}}$.
:::
...
::: {#def-sharp-causal-null}
## Sharp Causal Null Hypothesis

The **sharp causal null hypothesis** holds when there is no causal effect for *any* individual, i.e., $Y^{a=1} = Y^{a=0}$ for all individuals. The sharp null implies the null hypothesis of no average effect.
:::
...
::: {#def-fixed-vs-superpopulation}
## Fixed Population vs. Superpopulation

- **Fixed population**: The 20 individuals are the entire population of interest.
- **Random sample (superpopulation)**: The 20 individuals are a random sample from a larger "superpopulation."
:::
...
::: {.notes}
For example, suppose we randomly sample 20 individuals from a superpopulation where $\E{Y^{a=1}} = \E{Y^{a=0}} = 0.5$ (true null effect). We might observe $\hE{Y^{a=1}} = 0.55$ and $\hE{Y^{a=0}} = 0.45$ in our sample just by chance.
...
::: {#def-random-systematic-error}
## Random Error vs. Systematic Error

- **Random error**: Error due to sampling variability; reduced by larger sample sizes.
- **Systematic error (bias)**: Error due to flaws in study design; NOT reduced by larger samples.
:::
...
This is denoted $Y \ind A$.

When this equality does not hold, $A$ and $Y$ are **associated** or **dependent**.
:::

Association measures include the associational risk difference, risk ratio, and odds ratio.

::: {#def-associational-risk-difference}
## Associational Risk Difference

$$\Pr[Y = 1|A = 1] - \Pr[Y = 1|A = 0]$$
:::

::: {#def-associational-risk-ratio}
## Associational Risk Ratio

$$\Pr[Y = 1|A = 1] / \Pr[Y = 1|A = 0]$$
:::

::: {#def-associational-odds-ratio}
...
::: {#def-causation-vs-association}
## Causation vs. Association

- **Causation**: Compares the *same* population under different treatment values ("What if everyone had been treated?" vs. "What if everyone had been untreated?"), using counterfactual risks: $\Pr[Y^{a=1} = 1]$ vs. $\Pr[Y^{a=0} = 1]$.
- **Association**: Compares *different subsets* of the population ("What is the risk in those who were treated?" vs. "What is the risk in those who were untreated?"), using conditional risks: $\Pr[Y = 1|A = 1]$ vs. $\Pr[Y = 1|A = 0]$.
:::
