...
This chapter defines causal effects using the potential outcomes framework, also known as counterfactual outcomes. We distinguish between individual and average causal effects, introduce different measures of causal effects, discuss the role of random variability, and clarify the critical distinction between causation and association.

::: {.notes}
The content is based on @hernan2020causal [Chapter 1, pp. 3-12]. This chapter lays the foundation for all subsequent material by precisely defining what we mean by a "causal effect" using counterfactual outcomes.
:::
...
::: {.content-visible when-format="html"}
**Table 1.2**: Observed treatment and outcome (hypothetical data)

In reality, we observe only one outcome per person—the one corresponding to their actual treatment. Suppose 13 individuals received transplants and 7 did not. Among the 13 treated, 7 died. Among the 7 untreated, 3 died.
:::

The **associational risk** in the treated: $Pr[Y = 1|A = 1] = 7/13 \approx 0.54$

The **associational risk** in the untreated: $Pr[Y = 1|A = 0] = 3/7 \approx 0.43$
